...
void update()
{
    ...
    // lose a live and reset position
    if (g_current_scene->m_state.fell) {
        g_current_scene->m_state.fell = false;
        lives -= 1;
        if (g_current_scene == g_levelB) g_current_scene->m_state.player->set_position(glm::vec3(4.0f, 0.0f, 0.0f));
        else g_current_scene->m_state.player->set_position(glm::vec3(1.0f, 0.0f, 0.0f));
    }
